...
namespace ToonBoom.TBGImporter
{
    ...
    public partial class TBGImporter : ScriptedImporter
    {
        ...
        [Tooltip("Prefix each clip name with a common prefix")]
        ...
        [Header("Sub Assets")]
        [Tooltip("When disabled, the final prefab will not be provided")]
        public bool CreatePrefab = true;
        [Tooltip("When disabled, no animation clips will be created, so you can be sure that they aren't included in builds")]
        public bool CreateAnimationClips = true;
        [Tooltip("When 'ignored', textures will not be created from the TBG. This will still create Sprites, which can be used for reference when providing other TBG Store assets to the TBG Renderer. Embed will include the texture as a subasset, and External will creat the Texture2D asset seperately in the file system.")]
        public AssetCreationMode CreateTextures = AssetCreationMode.Embed;
        [Tooltip("When this TBG is imported, it will first check the Base Sprite Store for sprites of the same name to use in animations, and will not create any sprites for itself with the same name. This allows for multiple characters with shared sprites to not create redundant art.")]
        public LazyLoadReference<TBGStore> BaseSpriteStore;
        [Tooltip("Rename a substring in each GameObject with an alternative substring. This is useful for aligning your prefab with existing animations.")]
        public List<NameRename> GameObjectRenameLookup = new List<NameRename>();

        [Serializable]
        public struct NameRename
        {
            public String from;
            public String to;
        }
        ...
        public SpriteAtlas CreateSpriteAtlasAsset()
        {
            ...
            var assetsFolderName = name + " Assets";
            var combinedPath = Path.Combine(dir, assetsFolderName);
            if (!System.IO.Directory.Exists(combinedPath))
            {
                var newFolder = AssetDatabase.CreateFolder(dir, assetsFolderName);
            }
            AssetDatabase.CreateAsset(atlas, Path.Combine(dir, assetsFolderName, $"{name}.spriteatlas"));
            SpriteAtlas = atlas;
            EditorUtility.SetDirty(this);
            return SpriteAtlas;
        }

        public T AddAssetToFolder<T>(T generatedAsset) where T : UnityEngine.Object
        {
            var path = AssetDatabase.GetAssetPath(this);
            var dir = Path.GetDirectoryName(path);
            var name = Path.GetFileNameWithoutExtension(path);
            ...
            if (!System.IO.Directory.Exists(combinedPath))
            {
                AssetDatabase.CreateFolder(dir, assetsFolderName);
            }
            var assetPath = Path.Combine(dir, assetsFolderName, $"{generatedAsset.name}.asset");
...
#nullable disable
        ...
        {
            if (BaseSpriteStore.isSet && AssetDatabase.TryGetGUIDAndLocalFileIdentifier(BaseSpriteStore, out var guid, out long _))
            ...
            try
            {
                ...
                // Retrieve an existing sprite store
                Profiler.BeginSample("Create Sprite Assets");

                var spriteSheetToSprites = data.spriteSheets
                    .Select(spriteSheet =>
                    {
                        var splits = spriteSheet.resolution.Split('-');
                        var resolutionName = splits[0];
                        var paletteName = String.Join("-", splits.Skip(1));
                        var baseSprites = BaseSpriteStore.asset?.Resolutions
                            .Where(entry => entry.ResolutionName == resolutionName)
                            .Select(entry => entry.Palettes
                                .Where(entry => entry.PaletteName == paletteName)
                                .Select(entry => entry.Sprites)
                                .FirstOrDefault())
                            .FirstOrDefault()?
                            .ToDictionary(entry => entry.name, entry => entry);

                        Dictionary<string, Sprite> sprites;
                        if (CreateTextures == AssetCreationMode.Ignore)
                        {
                        ...
                        else if (spriteSheet.filename != null)
                        {
                            ...
                            sprites = spriteSheet.sprites
                                .Select(spriteSettings =>
                                {
                                    var spriteName = $"{spriteSheet.resolution}-{spriteSettings.name}";
                                    if (baseSprites != null && baseSprites.TryGetValue(spriteName, out var baseSprite))
                                    {
                                        // Quit early with existing sprite reference, don't create a new sprite or add sprite to this asset
                                        Debug.Log($"Skipped sprite {spriteSettings.name} because the Base Sprite Store already has that sprite");
                                        Debug.LogWarning($"It is recommended you re-export your TBG files with Individual Sprites option enabled, so more accurate pixels comparisons can be run");
                                        return (spriteSettings.name, sprite: baseSprite);
                                    }
                                    ...
                                    ctx.AddObjectToAsset(sprite.name, sprite);
                                    return (spriteSettings.name, sprite);
                                })
                                    .ToDictionary(entry => entry.name, entry => entry.sprite);
                            switch (CreateTextures)
                            {
                                case AssetCreationMode.Embed:
                                    ctx.AddObjectToAsset(
                                        textureGenerationOutput.texture.name,
                                        textureGenerationOutput.texture,
                                        textureGenerationOutput.thumbNail);
                                    break;
                                case AssetCreationMode.External:
                                    AddAssetToFolder(textureGenerationOutput.texture);
                                    break;
                            }
                        }
                        else
                        {
                            ...
                            sprites = spriteSheet.sprites
                                .Select(spriteSettings =>
                                {
                                    ...
                                    if (baseSprites != null && baseSprites.TryGetValue(spriteName, out var baseSprite))
                                    {
                                        // Compare all pixels (at a lower mip) for equality
                                        var mipLevel = 3;
                                        var pixels = textureGenerationOutput.texture.GetPixels32(mipLevel);
                                        var basePixels = baseSprite.texture.GetPixels32(mipLevel);
                                        if (pixels.Length == basePixels.Length)
                                        {
                                            var arePixelsEqual = true;
                                            for (var i = 0; i < pixels.Length; i++)
                                            {
                                                if (!pixels[i].Equals(basePixels[i]))
                                                {
                                                    arePixelsEqual = false;
                                                    break;
                ...
                var renameLookup = GameObjectRenameLookup.ToDictionary(entry => entry.from, entry => entry.to);
                ...
                TBGStore tbgStore;
                {
                    ...
                    var readNameToIndex = readNodes
                        ...
                        .ToLookup(entry => entry.name, entry => entry.index);
                    ...
                    tbgStore = ScriptableObject.CreateInstance<TBGStore>();
                    {
                        ...
                        tbgStore.ReadToDeformedSpriteEntries = nodeToDeformedSprites
                            ...
                            {
                                ReadIndex = readNameToIndex[entry.Key].First(),
                    ...
                    var skewEntries = skewNodes
                        ...
                        .Select(instantiated =>
                        {
                            var existingChildren = Enumerable
                                ...
                                .ToList();
                            var skewBase = new GameObject("_SkewBase").transform;
                            var skewCounter = new GameObject("_SkewCounter").transform;
                ...
                if (CreateAnimationClips)
                {
                    ...
                    var clips = stages
                        .Select(stage =>
                        {
                            ...
                            {
                                Name = ClipPrefix + stage.play.animation,
                                Framerate = Framerate,
                                Stepped = Stepped,
        ...
        private static GameObject CreateGameObject(List<NameRename> renameLookup, NodeSettings node, HashSet<string> usedNames, Transform parent)
        {
            var gameObjectName = node.name;
            foreach (var rename in renameLookup)
            {
                gameObjectName = gameObjectName.Replace(rename.from, rename.to);
            }
